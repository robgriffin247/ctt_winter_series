...
add_categories as (
    ...
    *,
    case
      when watts_max>=250 or wkg_max>=4.2 then 'A'
      when watts_max>=200 or wkg_max>=3.36 then 'B'
      when watts_max>=150 or wkg_max>=2.63 then 'C'
      when watts_max>0 or wkg_max>0 then 'D'
