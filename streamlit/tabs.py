...
def render_standings(results):
    ...
    def get_standings(
        results,
        selected_gender_category,
        selected_power_category,
        ...
        if selected_gender_category == "Mens":
            ...
            results = results.with_columns(
                pl.col("mixed_category").alias("power_category")
            )

        elif selected_gender_category == "Womens":
            results = results.filter(pl.col("gender") == "F")
            results = results.with_columns(
                pl.col("womens_category").alias("power_category")
            )
        ...
        if selected_power_category != "All":
            results = results.filter(
                pl.col("power_category") == selected_power_category
            )
    ...
    gender_category = c1.selectbox(
        "Gender Category",
        options=["Open", "Mens", "Womens"],
        key="standings_gender_category",
    )
    power_category = c2.selectbox(
        "Power Category",
        options=["All", "A", "B", "C", "D"],
        key="standings_power_category",
    )
    age_category = c3.selectbox(
        "Age Category",
        options=["All", "Jnr", "U23", "Snr", "Mas", "Vet", "50+", "60+", "70+", "80+"],
        key="standings_age_category",
    )
    round = c4.selectbox(
        "Round",
        options=["All", "Completed"] + results["round_id"].unique().sort().to_list(),
        key="standings_round",
    )
    clubs = c5.multiselect(
        "Club(s)",
        options=results[["club", "club_id"]].unique().sort(pl.col("club"))["club"],
        key="standings_clubs",
    )
    riders = c6.multiselect(
        "Rider(s)",
        options=results.filter(pl.col("club").is_in(clubs) if len(clubs) > 0 else True)[
            ["rider", "rider_id"]
        ]
        .unique()
        .sort(pl.col("rider"))["rider"],
        key="standings_riders",
    )

    standings = get_standings(
        results, gender_category, power_category, age_category, round, clubs, riders
    )
...
def render_results(results):
    ...
    gender_category = c1.selectbox(
        "Gender Category",
        options=["Open", "Mens", "Womens"],
        key="results_gender_category",
    )
    power_category = c2.selectbox(
        "Power Category",
        options=["All", "A", "B", "C", "D"],
        key="results_power_category",
    )
    age_category = c3.selectbox(
        "Age Category",
        options=["All", "Jnr", "U23", "Snr", "Mas", "Vet", "50+", "60+", "70+", "80+"],
        key="results_age_category",
    )
    ...
    riders = c6.multiselect(
        "Rider(s)",
        options=results[["rider", "rider_id"]].unique().sort(pl.col("rider"))["rider"],
        key="results_riders",
    )
    if riders != []:
        results = results.filter(pl.col("rider").is_in(riders))

    routes = c7.multiselect(
        "Route(s)",
        options=results[["route"]].unique().sort(pl.col("route"))["route"],
        key="results_route",
    )
    if routes != []:
        results = results.filter(pl.col("route").is_in(routes))

    rounds = c4.multiselect(
        "Round(s)",
        options=results[["round_id"]].unique().sort(pl.col("round_id"))["round_id"],
        key="results_round",
    )
    if rounds != []:
    ...
    if results.shape[0] == 0:
        ...
        st.dataframe(
            results[
                ...
                [pl.col("round_id"), pl.col("race_speed")], descending=[False, True]
            ),
            column_config={
                ...
                "race_speed": st.column_config.NumberColumn(
                    "Speed (km/h)", format="%.2f"
                ),
                ...
                "start_datetime_utc": st.column_config.DatetimeColumn(
        ...
        if len(riders) == 1:
            ...
            c1.metric(
                "Flat",
                f"{results.filter(pl.col('is_best_effort_in_round')).filter(pl.col('route_type')=='flat').shape[0]} of 4",
                border=True,
            )
            c2.metric(
                "Rolling",
                f"{results.filter(pl.col('is_best_effort_in_round')).filter(pl.col('route_type')=='rolling').shape[0]} of 2",
                border=True,
            )
            c3.metric(
                "Mountain",
                f"{results.filter(pl.col('is_best_effort_in_round')).filter(pl.col('route_type')=='mountain').shape[0]} of 1",
                border=True,
            )
...
def render_stats(results, christmas=False):
    ...
    def power_figure(data):
        ...
        y_metric = c2.selectbox(
            "Y-Axis Value",
            options=["Race Speed", "Race Time", "Segment Speed", "Segment Time"],
        )
        c_metric = c2.selectbox(
            "Colour by",
            options=[
                "Category",
                "Route" if "Race" in y_metric else "Segment",
                "Gender",
            ],
        )

        if "Race" in y_metric:
            selected_routes = c2.multiselect(
                "Route(s)",
                options=data[["route"]]
                .unique()
                .sort(pl.col("route"))["route"]
                .to_list(),
            )
            selected_segments = []
        ...
        if "Segment" in y_metric:
            selected_routes = []
            selected_segments = c2.multiselect(
                "Segment(s)",
                options=data[["segment"]]
                .unique()
                .sort(pl.col("segment"))["segment"]
                .to_list(),
            )

        selected_power_category = c2.selectbox(
            "Power Category", options=["All", "A", "B", "C", "D"]
        )
        ...
        fig = px.scatter(
            data.filter(
                pl.col("route").is_in(selected_routes)
                if len(selected_routes) > 0
                else True
            )
            .filter(
                pl.col("segment").is_in(selected_segments)
                if len(selected_segments) > 0
                else True
            )
            .filter(
                pl.col("mixed_category") == selected_power_category
                if selected_power_category != "All"
                else True
            )
    ...
    if not christmas or st.toggle("Bah! I don't like christmas!", value=False):
        c1.metric(
            "Riders 🚴‍♂️", f"{results[["rider_id"]].unique().shape[0]:,.0f}", border=True
        )
        ...
        c6.metric(
            "Everests Climbed 🏔️",
            f"{sum(results["route_elevation"])/8848:,.2f}",
            border=True,
        )
        ...
        c1.metric(
            "Santas 🎅",
            f"{results[["rider_id"]].unique().shape[0]:,.0f}",
            border=True,
            help="Riders",
        )
        c2.metric(
            "Sleds Raced 🛷", f"{results.shape[0]:,.0f}", border=True, help="Efforts"
        )
        c3.metric(
            "Gifts Received 🎁",
            f"{sum(results["is_new_pb"]):,.0f}",
            border=True,
            help="PBs beaten",
        )
        c4.metric(
            "Sleigh Rides Around the World 🌍",
            f"{distance/40075:,.2f}",
            border=True,
            help=f"{distance:,.0f} km",
        )
        ...
        c5.metric(
            ...
            f"{hours*60/132:,.1f}",
            border=True,
            help=f"{hours:,.0f} cumulative hours of riding; Christmas cracker Die Hard is 2h 12m",
        )
        c6.metric(
            "Mt Gunnbjørns Climbed 🏔️",
            f"{sum(results["route_elevation"])/3694:,.1f}",
            border=True,
            help=f"{sum(results["route_elevation"]):,.0f} meters climbed; Mt Gunnbjørn his the highest peak in the Arctic Circle at 3694 metres above sea level",
        )
        c7.metric(
            "Christmas Lights Lit 🎄",
            f"{kwhs/(5.65/100):,.0f} m",
            border=True,
            help=f"{kwhs:.0f} kW/hs; based on 7.6 W per 100 metres for all of December",
        )  # 5.65 per 100m; 5.65/100 = per metre
        c8.metric(
            "Logs Burned 🔥",
            f"{kwhs * 860.420 / 0.24 / 1900:,.1f} kg",
            border=True,
            help=f"{kwhs * 860.420 / 0.24:,.0f} calories",
        )  # 1900 cal/kg
        c9.metric(
            "Turkey Legs Eaten 🍗",
            f"{kwhs * 860.420 / 0.24 / 416:,.0f}",
            border=True,
            help=f"{kwhs * 860.420 / 0.24 / 266:,.0f} pizza slices",
        )
    ...
    for r in rounds.iter_rows(named=True):
        with st.expander(f"{r['round_id']}: {r['route']}, {r['date_range']}"):
            ...
            if r["mens"] is not None:
                st.dataframe(
                    winners.filter(pl.col("round_id") == r["round_id"])[
                        ["rider", "category", "gender"]
                    ]
                    .pivot(index="category", on="gender", values="rider")
                    .sort("category"),
                    column_config={
