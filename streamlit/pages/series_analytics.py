...
def login():
    password = st.text_input(
        "Password", value=None, type="password", help="Not password123"
    )
    st.session_state["login_state"] = (
        True if password == os.getenv("SERIES_ANALYTICS_SECRET") else False
    )
...
if st.session_state.get("login_state") != True:
    ...
    with duckdb.connect() as con:
        ...
        round_day_stats = con.sql(
            """
                                    ...
                                    order by 1, day(start_datetime_utc)"""
        ).pl()
        ...
        event_stats = con.sql(
            """
                                    ...
                                    from results 
                                    group by 1, 2, 3
                                    order by 1 desc, start_datetime_utc desc, event_id"""
        ).pl()
    ...
    stats_columns = {
        ...
        "start_datetime_utc": st.column_config.DatetimeColumn(
            "Date/Time (UTC)", format="DD-MM-YYYY hh:mm"
        ),
        ...
        "percent_of_efforts": st.column_config.NumberColumn(
            "% of Efforts", format="%.1f"
        ),
    ...
    with st.expander("Round & Day Stats", expanded=False):
        st.dataframe(
            round_day_stats.drop(pl.col("day_number", "date")),
    ...
    with st.expander("Efforts per Country", expanded=False):
        st.dataframe(
            rides_per_country[["country_name", "efforts", "percent_of_efforts"]],
            column_config=stats_columns,
        )
